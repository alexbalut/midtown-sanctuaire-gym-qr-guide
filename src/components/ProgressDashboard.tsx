...
export function ProgressDashboard({ gymSlug, gymColor, lang }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {rows.length === 0 ? (
        <p className="text-muted text-sm card p-5">{t.empty}</p>
      ) : (
            ...
            <li key={m.machineId} className="card p-4 sm:p-5 space-y-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right shrink-0 text-xs text-muted">
                  <p>
                    <span className="text-muted">{t.timesUsed}</span>
                    <br />
                    <strong className="text-navy text-base">{m.timesUsed}</strong>
                  </p>
                </div>
              </div>

              <dl className="grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5">
                  <dt className="text-xs text-muted">{t.lastUsed}</dt>
                ...
                {isCardio(m.category) ? (
                  <>
                    <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5">
                      <dt className="text-xs text-muted">{t.lastDuration}</dt>
                      <dd className="font-medium text-navy mt-0.5">
                        {m.lastDurationSec != null ? formatDuration(m.lastDurationSec) : "—"}
                      </dd>
                    </div>
                    <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5">
                      <dt className="text-xs text-muted">{t.bestDuration}</dt>
                      <dd className="font-medium mt-0.5" style={{ color: gymColor || "#C4A35A" }}>
                        {m.bestDurationSec != null ? formatDuration(m.bestDurationSec) : "—"}
                      </dd>
                    </div>
                    <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5">
                      <dt className="text-xs text-muted">{t.totalTime}</dt>
                      <dd className="font-medium text-navy mt-0.5">
                  ...
                  <>
                    <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5">
                      <dt className="text-xs text-muted">{t.totalSets}</dt>
                      <dd className="font-medium text-navy mt-0.5">{m.totalSets}</dd>
                    </div>
                    <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5">
                      <dt className="text-xs text-muted">{t.heaviest}</dt>
                      <dd className="font-medium mt-0.5" style={{ color: gymColor || "#C4A35A" }}>
                        {m.heaviestKg != null ? `${m.heaviestKg} kg` : "—"}
                      </dd>
                    </div>
                    <div className="rounded-xl bg-[var(--wash)] border border-border px-3 py-2.5 col-span-2">
                      <dt className="text-xs text-muted">{t.lastSession}</dt>
                      <dd className="font-medium text-navy mt-0.5 break-words">
                        {m.lastSetsSummary ?? "—"}
                      </dd>
                    </div>
                  </>
              ...
              {m.recent.length > 0 && (
                <div>
                  <p className="text-xs text-muted mb-2">{t.history}</p>
                  <ul className="space-y-1.5">
                    {m.recent.map((r, i) => (
                      ...
                      >
                        <span className="text-muted shrink-0">{formatDate(r.savedAt, lang)}</span>
                        <span className="text-navy text-right truncate">{r.summary}</span>
                      </li>
                    ))}
                  </ul>
                </div>
