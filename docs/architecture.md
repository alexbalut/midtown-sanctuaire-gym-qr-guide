...
Seed gym: `midtown-sanctuaire`, `primaryColor: #C4A35A`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `midtown-sanctuaire` (fallback: first gym).
...
| Workout + history | localStorage keys `midtown-sanctuaire-workout:v1:<slug>` and `midtown-sanctuaire-workout-history:v1:<slug>` |
