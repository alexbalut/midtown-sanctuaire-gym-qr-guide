...
Members never log in. `/` is the in-product gym home for **Midtown Sanctuaire Montréal**.
...
3. Friendly URLs: `/m/midtown-sanctuaire/lat-pulldown`.
...
Keyed by gym slug (`midtown-sanctuaire`). No member account.

| Key | Contents |
|-----|----------|
| `midtown-sanctuaire-workout:v1:<slug>` | In-progress session JSON |
| `midtown-sanctuaire-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
